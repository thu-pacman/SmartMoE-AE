...
def draw(fig, axs, nnodes, mbs, lat, layer_idx):
    ...
    with open(f'{root}/plotting/from_exec/fig13/freq100.log', 'r') as f:
        line = f.readlines()[0]
    ...
    with open(f'{root}/plotting/from_exec/fig13/freq100_star.log', 'r') as f:
        lines = f.readlines()
    ...
    with open(f'{root}/plotting/from_exec/fig13/freq10.log', 'r') as f:
        line = f.readlines()[0]
        data = json.loads(line)
        t0 = data['FastMoE'][1:]
        t2 = data['SmartMoE'][1:]
    ...
    with open(f'{root}/plotting/from_exec/fig13/freq10_star.log', 'r') as f:
        lines = f.readlines()
        x_data = [int(v) for v in lines]
        y_data = [t2[i//10] for i in x_data]
        x_data = np.array(x_data)

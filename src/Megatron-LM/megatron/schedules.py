...
def forward_step(forward_step_func, data_iterator, model, input_tensor, losses_reduced, is_first_micro_batch=True, is_last_micro_batch=True):
    ...
    timers('fwd-compute').start()
    if args.fmoefy:
        output_tensor, loss_func, bal_loss = forward_step_func(data_iterator, model, is_first_micro_batch=is_first_micro_batch, is_last_micro_batch=is_last_micro_batch)
        bal_loss = bal_loss / get_num_microbatches()
    else:
        output_tensor, loss_func = forward_step_func(data_iterator, model)
    timers('fwd-compute').stop()
    timers('fwd-loss-func').start()
    if mpu.is_pipeline_last_stage():
        output_tensor = loss_func(output_tensor)
        loss, loss_reduced = output_tensor
        output_tensor = loss / get_num_microbatches()
        losses_reduced.append(loss_reduced)
    timers('fwd-loss-func').stop()
...
def backward_step(optimizer, input_tensor, output_tensor, output_tensor_grad, bal_loss=None):
    ...
    # print("rank {} loss_scale {}".format(rank,loss_scale), flush=True)
    ...
    if output_tensor_grad is None:
        ...
        # bal_loss = optimizer.scale_loss(bal_loss)
        if bal_loss:
...
def forward_backward_no_pipelining(forward_step_func, data_iterator, model,
        ...
        for i in range(get_num_microbatches() - 1):
            if args.fmoefy:
                output_tensor, bal_loss = forward_step(forward_step_func, data_iterator, model,
                                            input_tensor, losses_reduced, is_first_micro_batch=(i==0), is_last_micro_batch=False)
            else:
    ...
    if args.fmoefy: 
        output_tensor, bal_loss = forward_step(forward_step_func, data_iterator, model,
                                     input_tensor, losses_reduced, is_first_micro_batch=(get_num_microbatches()==1), is_last_micro_batch=True)
    else:
        output_tensor = forward_step(forward_step_func, data_iterator, model,
                                     input_tensor, losses_reduced)
        bal_loss = None
...
def forward_backward_pipelining_without_interleaving(forward_step_func, data_iterator,
    ...
    for i in range(num_warmup_microbatches):
        ...
        if args.fmoefy:
            output_tensor, bal_loss = forward_step(forward_step_func, data_iterator, model,
                                         input_tensor, losses_reduced, is_first_micro_batch=(i==0), is_last_micro_batch=(i==num_microbatches - 1))
        else:
            output_tensor = forward_step(forward_step_func, data_iterator, model,
                                         input_tensor, losses_reduced)
            bal_loss = None
    ...
    for i in range(num_microbatches_remaining):
        ...
        if args.fmoefy:
            output_tensor, bal_loss = forward_step(forward_step_func, data_iterator, model,
                                         input_tensor, losses_reduced, is_first_micro_batch=(num_warmup_microbatches == 0 and i == 0), is_last_micro_batch=last_iteration)
        else:
            output_tensor = forward_step(forward_step_func, data_iterator, model,
                                         input_tensor, losses_reduced)
            bal_loss = None
